...
# Title & Info
cells.append(nbf.v4.new_markdown_cell("""# Major Power Outages Analysis: Climate Anomalies and Grid Resilience
...
We are working with the **U.S. Major Power Outages dataset (2000–2016)**, compiled by the U.S. Department of Energy (DOE) and curated by Purdue University LASCI. The dataset contains **1,534 rows** and **56 columns**, where each row represents a major power outage affecting a single U.S. state.

We chose this dataset because power outages are a critical public infrastructure and safety issue directly impacted by extreme weather and climate change. We hope to understand how meteorological conditions, regional differences, and grid characteristics affect how long outages last and how severe they become.
...
- `OUTAGE.RESTORATION.DATE` and `OUTAGE.RESTORATION.TIME`: The date and time when electricity was restored.
- `OUTAGE.DURATION`: Total restoration duration in minutes.
- `CAUSE.CATEGORY`: The main trigger of the outage (e.g., severe weather, intentional attack, equipment failure).
- `ANOMALY.LEVEL`: Oceanic Niño Index (ONI) measuring sea surface temperature anomalies (°C).
- `CLIMATE.REGION`: U.S. climate region (e.g., Northeast, South, West).
- `CUSTOMERS.AFFECTED`: Total number of customers who lost power.
- `DEMAND.LOSS.MW`: Peak power demand lost in Megawatts.
- `RES.PRICE` & `TOTAL.SALES`: Average residential electricity price (cents/kWh) and total electricity consumed (MWh).
...
### Data Cleaning Steps
1. **Skipped Metadata Rows**: The raw Excel file has title and variable description rows at the top. We loaded the file starting at row index 5 to extract clean column headers and dropped the secondary physical units row.
2. **Combined Date and Time**: We merged `OUTAGE.START.DATE` and `OUTAGE.START.TIME` into a single `pd.Timestamp` column named `OUTAGE.START`. Similarly, we merged `OUTAGE.RESTORATION.DATE` and `OUTAGE.RESTORATION.TIME` into `OUTAGE.RESTORATION`.
3. **Validated Durations**: We verified that `(OUTAGE.RESTORATION - OUTAGE.START)` in minutes matches `OUTAGE.DURATION` (minor 60-minute differences correspond to daylight saving time shifts).
4. **Converted Types**: Converted numeric columns (`YEAR`, `MONTH`, `OUTAGE.DURATION`, `CUSTOMERS.AFFECTED`, `DEMAND.LOSS.MW`, `ANOMALY.LEVEL`, prices, population density) to numeric types.
5. **Feature Engineering**: Created `OUTAGE.DURATION.HOURS = OUTAGE.DURATION / 60`, `LOG_DURATION = np.log1p(OUTAGE.DURATION)`, `START_MONTH`, `START_HOUR`, and an indicator `IS_SEVERE_OUTAGE = (OUTAGE.DURATION >= 1440)` for outages lasting 24+ hours.
"""))

cells.append(nbf.v4.new_code_cell("""df_raw = pd.read_excel('data/outage.xlsx', sheet_name='Masterdata', header=5)
...
numeric_cols = [
    ...
    'RES.SALES', 'COM.SALES', 'IND.SALES', 'TOTAL.SALES', 'POPULATION', 
    'POPDEN_URBAN', 'POPDEN_RURAL', 'AREAPCT_URBAN', 'PCT_LAND', 'PC.REALGSP.STATE'
]
...
def combine_dt(date_series, time_series):
    ...
    times = time_series.apply(lambda t: str(t) if pd.notnull(t) else np.nan)
    res = []
...
df['OUTAGE.START'] = combine_dt(df['OUTAGE.START.DATE'], df['OUTAGE.START.TIME'])
df['OUTAGE.RESTORATION'] = combine_dt(df['OUTAGE.RESTORATION.DATE'], df['OUTAGE.RESTORATION.TIME'])
...
display(df[['U.S._STATE', 'CLIMATE.REGION', 'CAUSE.CATEGORY', 'OUTAGE.START', 'OUTAGE.RESTORATION', 'OUTAGE.DURATION.HOURS', 'CUSTOMERS.AFFECTED']].head(5))
"""))
...
fig_duration.update_layout(xaxis_title='Duration (Hours)', yaxis_title='Log Count', template='simple_white+dsc80')
fig_duration.write_html('assets/univariate_duration_distribution.html', include_plotlyjs='cdn')
fig_duration.show()
...
fig_cause.update_layout(xaxis_tickangle=-30, yaxis_title='Number of Outages', template='simple_white+dsc80')
fig_cause.write_html('assets/univariate_cause_distribution.html', include_plotlyjs='cdn')
fig_cause.show()
"""))
...
**Plot 3 (Duration Across Cause Categories)**: Severe weather outages have a median restoration time of 41.0 hours (mean of 64.7 hours), whereas intentional attacks have a median duration of only 1.2 hours. Weather damages physical power lines over large areas, requiring extensive on-site crew repairs.

**Plot 4 (Outage Volume and Mean Duration by Climate Region)**: The Northeast experiences both the highest volume of major outages and high average durations, while the West experiences many events with shorter average restoration times.
"""))
...
fig_cause_dur.update_layout(xaxis_tickangle=-30, showlegend=False, template='simple_white+dsc80')
fig_cause_dur.write_html('assets/bivariate_cause_duration_box.html', include_plotlyjs='cdn')
fig_cause_dur.show()
...
fig_reg.add_trace(go.Bar(x=reg_stats['CLIMATE.REGION'], y=reg_stats['Count'], name='Outage Count', marker_color='#1f77b4'), secondary_y=False)
fig_reg.add_trace(go.Scatter(x=reg_stats['CLIMATE.REGION'], y=reg_stats['Mean_Duration'], name='Mean Duration (Hours)', mode='lines+markers', line=dict(color='#d62728', width=3)), secondary_y=True)
fig_reg.update_layout(title='Outage Frequency and Mean Duration by Climate Region', xaxis_tickangle=-30, template='simple_white+dsc80')
...
fig_reg.write_html('assets/bivariate_region_volume_duration.html', include_plotlyjs='cdn')
fig_reg.show()
"""))
...
obs_table = df.pivot_table(index='CAUSE.CATEGORY', columns='cust_missing', aggfunc='size', fill_value=0)
...
for _ in range(3000):
    ...
    tab = df.assign(shuffled=shuffled).pivot_table(index='CAUSE.CATEGORY', columns='shuffled', aggfunc='size', fill_value=0)
    props = tab / tab.sum(axis=0)
    tvd_sims.append(tvd(props[False], props[True]))

p_val_tvd = np.mean(np.array(tvd_sims) >= obs_tvd)
print(f"CUSTOMERS.AFFECTED missingness vs CAUSE.CATEGORY: Obs TVD = {obs_tvd:.4f}, p-value = {p_val_tvd:.4f}")
print("Conclusion: Reject Null. Missingness of CUSTOMERS.AFFECTED depends on CAUSE.CATEGORY (MAR).")

# 2. Permutation Test: DEMAND.LOSS.MW missingness vs PCT_LAND (Difference in Means)
df['demand_missing'] = df['DEMAND.LOSS.MW'].isnull()
valid_land = df[['demand_missing', 'PCT_LAND']].dropna()
obs_diff_land = np.abs(valid_land.groupby('demand_missing')['PCT_LAND'].mean().diff().iloc[-1])

land_sims = []
for _ in range(3000):
    shuffled = np.random.permutation(valid_land['demand_missing'])
    diff = np.abs(valid_land.assign(shuffled=shuffled).groupby('shuffled')['PCT_LAND'].mean().diff().iloc[-1])
    land_sims.append(diff)

p_val_land = np.mean(np.array(land_sims) >= obs_diff_land)
print(f"\\nDEMAND.LOSS.MW missingness vs PCT_LAND: Obs Mean Diff = {obs_diff_land:.4f}%, p-value = {p_val_land:.4f}")
print("Conclusion: Fail to Reject Null (p > 0.05). DEMAND.LOSS.MW missingness does not depend on state land area %.")
"""))
...
cells.append(nbf.v4.new_code_cell("""hypo_df = df[df['CAUSE.CATEGORY'].isin(['severe weather', 'intentional attack', 'equipment failure'])].dropna(subset=['OUTAGE.DURATION']).copy()
...
print(f"Severe Weather Mean Duration: {means[True]:.1f} mins ({means[True]/60:.1f} hours)")
print(f"Other Causes Mean Duration:   {means[False]:.1f} mins ({means[False]/60:.1f} hours)")
print(f"Observed Difference in Means: {obs_diff:.2f} mins ({obs_diff/60:.2f} hours)")

np.random.seed(42)
perm_diffs = []
for _ in range(5000):
    shuffled = np.random.permutation(hypo_df['is_severe_weather'])
    shuff_means = hypo_df.assign(shuffled=shuffled).groupby('shuffled')['OUTAGE.DURATION'].mean()
...
fig_hypo = px.histogram(
    x=perm_diffs,
    nbins=40,
    title='Empirical Distribution of Mean Difference under H₀ vs Observed Difference',
    labels={'x': 'Difference in Mean Duration (Minutes)'}
)
fig_hypo.add_vline(x=obs_diff, line_dash='dash', line_color='red', annotation_text=f'Observed: +{obs_diff/60:.1f} hrs')
...
model_data = df.dropna(subset=['LOG_DURATION', 'CLIMATE.REGION', 'CAUSE.CATEGORY', 'ANOMALY.LEVEL', 'RES.PRICE']).copy()
...
adv_df['GRID_STRESS_INDEX'] = (adv_df['RES.PRICE'] * adv_df['TOTAL.SALES']) / (adv_df['POPULATION'] + 1.0)

std_features = ['ANOMALY.LEVEL', 'RES.PRICE', 'TOTAL.PRICE', 'MONTH_SIN', 'MONTH_COS', 'HOUR_SIN', 'HOUR_COS']
q_features = ['POPULATION', 'TOTAL.SALES', 'POPDEN_RATIO', 'GRID_STRESS_INDEX', 'PC.REALGSP.STATE', 'AREAPCT_URBAN']
...
adv_preprocessor = ColumnTransformer([
    ...
    ('q', QuantileTransformer(n_quantiles=100, output_distribution='normal', random_state=42), q_features),
    ('cat', OneHotEncoder(drop='first', sparse_output=False, handle_unknown='ignore'), cat_features_adv)
])
...
grid_search = GridSearchCV(rf_pipe, param_grid, cv=5, scoring='neg_root_mean_squared_error', n_jobs=-1)
...
if p_val_fairness >= 0.05:
    print("Conclusion: Fail to Reject Null (p >= 0.05). We do not find evidence of unfairness across climate regions.")
else:
    print("Conclusion: Reject Null (p < 0.05). The model exhibits significant difference in error across regions.")
"""))
...
print("Generated clean human-written project.ipynb and template.ipynb successfully!")
